...
function prepareInterviewData() {
  var quota_data_temp = JSON.parse(Dest_Airlines_Quota);
  ...
  for (i = 0; i < interview_data_temp.length; i++) {
    ...
    )
    {
      if (interview["Dest"] &&  interview["Flight"]) {
        var Dest_Airlines = '"Dest_Airlines"' + ":" + '"' +  interview["Dest"] + "-" + interview["Flight"].substring(0,2) + '", ';
        
        var InterviewEndDate = '"InterviewEndDate"' + ":" + '"' +  interview["InterviewEndDate"] ;
        var str = '{' + Dest_Airlines + InterviewEndDate + '"}';
  ...
  for (i = 0; i < today_flight_list.length; i++) {
    ...
    for (j = 0; j < quota_data.length; j++) {
      ...
      if ((quota.Dest_Airlines == flight.Dest_Airlines) && (quota.Quota>0))
      {
