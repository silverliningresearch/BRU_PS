...
function notDeparted(flight_time) {
  var current_time = new Date().toLocaleString('en-SG', { timeZone: 'Europe/Brussels', hour12: false});
  //15:13:27
...
function prepareInterviewData() {
  var flight_list_temp  = JSON.parse(BRU_Departures_Flight_List_Raw);
  ...
  for (i = 0; i < flight_list_temp.length; i++) {
    ...
    { 
      if (flight.Flight == "LX781") 
      {
        flight.Airline = "Swiss";
      }
